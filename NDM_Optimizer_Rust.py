"""
NDM (Net Deck Multiplier) Optimizer
=====================================
Simulates and optimizes deck configurations to maximise the Net Deck Multiplier.

Usage:
    python ndm_optimizer.py
    Deck configs are defined in the DECKS list near the bottom of the file.
    Parameters for cores can be altered around line 100.
    Please see https://docs.google.com/spreadsheets/d/1S98nbzfAVJpu0KNjJQQRH6br63SWRXJEoDGO_6JUnu8/edit?usp=drivesdk 
    for (probably) accurate data on the latest deck balance numbers for Wold's.

Card-type key used in grid displays:
    R = Row    C = Col    S = Surr
    ^ = DirGreed(up)    v = DirGreed(down)
    < = DirGreed(left)  > = DirGreed(right)
    e = EvoGreed    o = SurrGreed    . = generic_greed
"""
...
# Use this for "extra" core slots you can add to a deck. By default, deckmod = 1 because
# you can use the core expertise in wold's to add a free core when crafting, and then
# remove it using a table you find in dungeons, letting you apply a bonus core of whatever 
...
MULT_EVO_GREED:       float = 0 # turning these to "0" disables them.
MULT_SURR_GREED:      float = 0
MULT_DIR_GREED_DIAG_UP:   float = 0   # NE / NW diagonal greed
MULT_DIR_GREED_DIAG_DOWN: float = 0   # SE / SW diagonal greed

MULT_PURE_BASE:   float = 1.0
MULT_PURE_SCALE:  float = 0.1
MULT_EQUILIBRIUM: float = 3.0
MULT_FOIL:        float = 2.8  # this is the shiny core
...
EXPERIMENTAL_BOOST:           float = 0.4

DELUXE_COUNTED_AS_REGULAR: bool = False  # False → deluxe cards don't count toward min_regular constraint

# Display options
BALANCE_DISPLAY:    bool = True 
HEATMAP_DISPLAY:    bool = True

HNS_Q: float = 1.5   # Hill Number Score exponent; q=2 → EDP (Simpson reciprocal × NDM)
                      # q→1 → Shannon-weighted NDM;  q=0 → NDM × card count
SPREAD_METRIC: bool = False   # enables a metric to measure deck spread, but it kind of sucks
...
#   Config 1 — no greed cards    (max_greed=0,  min_regular=-1)
#   Config 2 — min 14 regular    (min_regular=14, max_greed=-1)
#   Config 3 — min 8 regular     (min_regular=8,  max_greed=-1)
#   Config 4 — unconstrained     (min_regular=-1, max_greed=-1)
# If min_regular > deck slot count, that config collapses to max_greed=0.
FULL_TEST_PANEL: bool = True
...
# Each config is (label, min_regular, max_greed)
...
def _deck_with_constraints(deck: Deck, min_regular: int, max_greed: int) -> Deck:
    """Return a shallow copy of deck with overridden constraints (slots unchanged)."""
    return Deck(deck.slots, deck.core_slots, deck.n_arcane, min_regular, max_greed, deck.name)


def _constraint_str(deck: Deck) -> str:
    n = len(deck.slots)
    min_reg_overridden = (
        deck.min_regular >= 0
        and deck.max_greed  >= 0
        and deck.min_regular + deck.max_greed > n
    )
    parts = []
    if deck.min_regular >= 0:
        note = " (overridden)" if min_reg_overridden else ""
        parts.append(f"min {deck.min_regular} regular{note}")
    if deck.max_greed >= 0:
        parts.append(f"max {deck.max_greed} greed")
    ...
    for card_class in CardClass:
        ...
        if verbose:
            ...
            print(f"  Class : {card_class.value.upper()}   Deck : {deck.name}"
                  f"   Constraint : {_constraint_str(deck)}")
...
def _report(card_class: CardClass, best: dict, deck: Deck) -> None:
    ...
    print(f"  Constraint    : {_constraint_str(deck)}")
    breakdown = "  ".join(
...
def _write_overview_block(
    ...
    for deck in decks:
        ...
        for i, (lbl, min_reg, max_greed) in enumerate(_get_test_configs(deck)):
            ...
            cdeck    = _deck_with_constraints(deck, min_reg, max_greed)
...
def _write_class_panel(
    ...
    for lbl, val in [
        ...
        ("Breakdown",  breakdown),
        ("Constraint", _constraint_str(deck)),
    ]:
...
# ──────────────────────────────────────────────────────────────────────────────
# Deck configurations  (you can add more)
# ──────────────────────────────────────────────────────────────────────────────

# You can add more decks at the bottom of this list. They are pretty self explanatory.
# Importantly, the "slots" field takes all the points in a grid where slots would be in 
