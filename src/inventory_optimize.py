...
def simulate_inventory(
    ...
    # ARCANE placements are "treated like regulars" — included in the EVO-no-FOIL
    # branch alongside positional/deluxe/typeless. (Old code added deck.n_arcane
    # as a phantom integer here; that fudge is gone.)
    if card_class == CardClass.EVO:
        n_ns = len(greed) if foil_active else (
            len(positional) + len(deluxe) + len(typeless) + len(arcane) + len(greed)
        )
    else:
        n_ns = len(greed)
    n_deluxe = len(deluxe)
...
def _try_pluto_swap_inventory(
    ...
    # n_ns mirrors simulate_inventory's rule: EVO-no-FOIL counts every non-greed
    # placement (including arcanes); other branches count greed only.
